...
public class PlainTextExportAddOn implements ExportAddOn {
	...
	private ExportOrder[] orders= new ExportOrder[]{ExportOrder.FoldersActions, ExportOrder.ProjectsActions};
	...
	public void export(GTDModel model, ActionsCollection collection, OutputStream out,
			ExportOrder order, FileFilter ff, boolean compact) throws Exception {
		...
		while (it.hasNext()) {
			...
			if (o instanceof Folder) {
				Folder f= (Folder)o;
				if (f.isProject()) {
					if (order== ExportOrder.ProjectsActions) {
						pw.print("@PROJECT: ");
				...
				} else if (order == ExportOrder.FoldersActions) {
					pw.print("@LIST: ");
					pw.println(f.getName());
					pw.print("@ID: ");
					pw.println(f.getId());
