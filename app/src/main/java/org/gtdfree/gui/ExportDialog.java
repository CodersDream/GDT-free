...
public class ExportDialog extends JDialog {
	...
	private void initialize() {
		...
		orderCombo.setRenderer(new DefaultListCellRenderer() {
			...
			public Component getListCellRendererComponent(JList list,
					Object value, int index, boolean isSelected,
					boolean cellHasFocus) {
				if (value instanceof ExportOrder) {
					value= ((ExportOrder)value).getDisplayName();
				}
	...
	protected void print() {
		try {
			...
			pdfExport.export(getGtdModel(), ac, out, (ExportOrder)orderCombo.getSelectedItem(), export.getFileFilters()[0], true);
	...
	protected void export() {
		...
		if (JFileChooser.APPROVE_OPTION==getFileChooser().showSaveDialog(this)) {
			...
			try {
				...
				export.export(getGtdModel(), ac, out, (ExportOrder)orderCombo.getSelectedItem(), fff, true);
	...
	protected void setExport(ExportAddOn selectedItem) {
		export= selectedItem;
		ExportOrder[] o= export.getSupportedExportOrders();
